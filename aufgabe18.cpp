...
Date::Date(const string &s) {
    _day   = std::stoi(s.substr(0, 2));
    _month = std::stoi(s.substr(3, 2));
    _year  = std::stoi(s.substr(6, 4));
}

// hier Konvertierkonstruktor für C-String
Date::Date(const char *cstr) {
    std::string s(cstr);   // C‑String in std::string umwandeln
    _day   = std::stoi(s.substr(0, 2));
    _month = std::stoi(s.substr(3, 2));
    _year  = std::stoi(s.substr(6, 4));
}
...
void Library::search_medium(string search_word) {
    ...
    for (int i = 0; i < (int)media.size(); i++) {
        Medium* m = media[i];
        if (m->get_title().find(search_word) != string::npos) {
            cout << "Medium " << i << ":\n";
            m->print();
        }
    }
...
void Library::check_out_medium(int number, Person &p, Date d) {
    if (number < 0 || number >= (int)media.size()) {
        cout << "Ungueltige Nummer!\n";
        return;
    }
...
void Library::print() const {
    cout << "Bibliothekskatalog:\n";
    cout << "---------------------------------------\n";
    cout << "\n"; 
    for (int i = 0; i < (int)media.size(); i++) {
        const Medium* m = media[i];
        cout << "Medium " << i << ":\n";
        m->print();
    }
    cout << "---------------------------------------\n";
...
int main(int argc, char *argv[]) 
{
    ...
    // Medien ausleihen
    cout << "\n";
    library.check_out_medium(1, p1, "04.05.2023");
